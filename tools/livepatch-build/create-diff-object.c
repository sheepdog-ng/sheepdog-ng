...
struct special_section {
	const char *name;
	int (*group_size)(struct livepatch_elf *kelf, int offset);
};
...
static const char *status_str(enum status status)
{
	switch(status) {
	case NEW:
		return "NEW";
...
static struct section *find_section_by_index(struct list_head *list, unsigned int index)
{
	struct section *sec;
...
static struct section *find_section_by_name(struct list_head *list, const char *name)
{
	struct section *sec;
...
static struct symbol *find_symbol_by_index(struct list_head *list, size_t index)
{
	struct symbol *sym;
...
static struct symbol *find_symbol_by_name(struct list_head *list, const char *name)
{
	struct symbol *sym;
 ...
 * Functions
 * **********/
static void livepatch_create_rela_list(struct livepatch_elf *kelf, struct section *sec)
{
	...
	while (rela_nr--) {
		...
		if (rela->sym->sec &&
			...
			rela->string = (char *)rela->sym->sec->data->d_buf + rela->addend;
			if (!rela->string)
...
static void livepatch_create_section_list(struct livepatch_elf *kelf)
{
	Elf_Scn *scn = NULL;
	struct section *sec;
...
static void livepatch_create_symbol_list(struct livepatch_elf *kelf)
{
...
static void livepatch_find_fentry_calls(struct livepatch_elf *kelf)
{
	struct symbol *sym;
	struct rela *rela;
...
static struct livepatch_elf *livepatch_elf_open(const char *name)
{
	Elf *elf;
	int fd;
	struct livepatch_elf *kelf;
	struct section *sec;
	...
	kelf->elf = elf;
	kelf->fd = fd;
	livepatch_create_section_list(kelf);
	livepatch_create_symbol_list(kelf);
	...
	list_for_each_entry(sec, &kelf->sections, list) {
		...
		livepatch_create_rela_list(kelf, sec);
	}

	livepatch_find_fentry_calls(kelf);
	return kelf;
}
 ...
 */
static int livepatch_mangled_strcmp(char *s1, char *s2)
{
	while (*s1 == *s2) {
		if (!*s1)
			return 0;
...
static int rela_equal(struct rela *rela1, struct rela *rela2)
{
	...
	if (is_special_static(rela1->sym))
		return !livepatch_mangled_strcmp(rela1->sym->name,
					      rela2->sym->name);

	return !strcmp(rela1->sym->name, rela2->sym->name);
}

static void livepatch_compare_correlated_rela_section(struct section *sec)
{
	struct rela *rela1, *rela2 = NULL;

	rela2 = list_entry(sec->twin->relas.n.next, struct rela, list);
...
static void livepatch_compare_correlated_nonrela_section(struct section *sec)
{
...
static void livepatch_compare_correlated_section(struct section *sec)
{
	struct section *sec1 = sec, *sec2 = sec->twin;
	...
	if (sec1->sh.sh_type != sec2->sh.sh_type ||
		...
		log_debug("%s section header details differ\n", sec1->name);
	...
	if (is_rela_section(sec))
		livepatch_compare_correlated_rela_section(sec);
	else
		livepatch_compare_correlated_nonrela_section(sec);
out:
 ...
 *
 */
static int livepatch_warn_only_change(struct section *sec)
{
	...
	for (offset = 0; offset < size; offset += length) {
		...
		if (insn1.length != insn2.length) {
            log_normal("insn length mismatch\n");
			return 0;
        }

		if (!memcmp((char *)start1 + offset, (char *)start2 + offset,
			    length))
			continue;
		...
		if (insn1.opcode.value != 0xba || insn2.opcode.value != 0xba) {
            log_normal("opcode mismatch %d %d\n", (int)insn1.opcode.value, (int)insn2.opcode.value);
			return 0;
        }
		warnonly = 1;
	}
...
static void livepatch_compare_sections(struct list_head *seclist)
{
	struct section *sec;
	...
	list_for_each_entry(sec, seclist, list) {
		if (sec->twin)
			livepatch_compare_correlated_section(sec);
		else
			sec->status = NEW;
	}
	...
	list_for_each_entry(sec, seclist, list) {
		if (livepatch_warn_only_change(sec)) {
...
static void livepatch_compare_correlated_symbol(struct symbol *sym)
{
...
static void livepatch_compare_symbols(struct list_head *symlist)
{
	struct symbol *sym;

	list_for_each_entry(sym, symlist, list) {
		if (sym->twin)
			livepatch_compare_correlated_symbol(sym);
		else
			sym->status = NEW;
...
static void livepatch_correlate_sections(struct list_head *seclist1, struct list_head *seclist2)
{
...
static void livepatch_correlate_symbols(struct list_head *symlist1, struct list_head *symlist2)
{
...
static void livepatch_compare_elf_headers(Elf *elf1, Elf *elf2)
{
	GElf_Ehdr eh1, eh2;
...
static void livepatch_check_program_headers(Elf *elf)
{
	size_t ph_nr;
...
static void livepatch_mark_grouped_sections(struct livepatch_elf *kelf)
{
	...
	list_for_each_entry(groupsec, &kelf->sections, list) {
		...
		end = (unsigned int *)((char *)groupsec->data->d_buf
                                     + groupsec->data->d_size);
 ...
 */
static void livepatch_rename_mangled_functions(struct livepatch_elf *base,
				     struct livepatch_elf *patched)
{
	...
	list_for_each_entry(sym, &patched->symbols, list) {
		...
		list_for_each_entry(basesym, &base->symbols, list) {
			if (!livepatch_mangled_strcmp(basesym->name, sym->name)) {
				found = 1;
				break;
			}
		}
...
static char *livepatch_section_function_name(struct section *sec)
{
	if (is_rela_section(sec))
		sec = sec->base;
 ...
 */
static struct symbol *livepatch_find_static_twin(struct section *sec,
					      struct symbol *sym)
{
	struct rela *rela;
	...
	list_for_each_entry(rela, &sec->twin->relas, list) {
		...
		if (livepatch_mangled_strcmp(rela->sym->name, sym->name))
			continue;
...
static int livepatch_is_normal_static_local(struct symbol *sym)
{
	if (sym->type != STT_OBJECT || sym->bind != STB_LOCAL)
		return 0;
 ...
 *   well.
 */
static void livepatch_correlate_static_local_variables(struct livepatch_elf *base,
					     struct livepatch_elf *patched)
{
	...
	list_for_each_entry(sym, &base->symbols, list) {
		...
		if (!livepatch_is_normal_static_local(sym))
			continue;
	...
	list_for_each_entry(sec, &base->sections, list) {
		...
		list_for_each_entry(rela, &sec->relas, list) {
			...
			patched_sym = livepatch_find_static_twin(sec, sym);
			if (!patched_sym)
				DIFF_FATAL("reference to static local variable %s in %s was removed",
					   sym->name,
					   livepatch_section_function_name(sec));
		...
		list_for_each_entry(rela, &sec->relas, list) {
			...
			if (!found)
				DIFF_FATAL("static local %s has been correlated with %s, but patched %s is missing a reference to it",
					   ...
					   livepatch_section_function_name(sec->twin));
		}
	}
...
static void livepatch_correlate_elfs(struct livepatch_elf *kelf1, struct livepatch_elf *kelf2)
{
	livepatch_correlate_sections(&kelf1->sections, &kelf2->sections);
	livepatch_correlate_symbols(&kelf1->symbols, &kelf2->symbols);
}

static void livepatch_compare_correlated_elements(struct livepatch_elf *kelf)
{
	/* lists are already correlated at this point */
	livepatch_compare_sections(&kelf->sections);
	livepatch_compare_symbols(&kelf->symbols);
}
 ...
 * so that the existing object/function in sheep can be linked to.
 */
static void livepatch_replace_sections_syms(struct livepatch_elf *kelf)
{
	struct section *sec;
	struct rela *rela;
	struct symbol *sym;
		...
		list_for_each_entry(rela, &sec->relas, list) {
			...
			list_for_each_entry(sym, &kelf->symbols, list) {
				...
				if (!is_text_section(sym->sec) &&
					 ...
					 *    of its data; and
					 *
					 * 2) Because livepatch doesn't allow data
					 *    sections to change,
...
static void livepatch_dump_kelf(struct livepatch_elf *kelf)
{
	struct section *sec;
	struct symbol *sym;
	struct rela *rela;
...
static void livepatch_check_fentry_calls(struct livepatch_elf *kelf)
{
	struct symbol *sym;
	int errs = 0;
...
static void livepatch_verify_patchability(struct livepatch_elf *kelf)
{
	struct section *sec;
	int errs = 0;
...
static void livepatch_include_symbol(struct symbol *sym, int recurselevel)
{
	struct rela *rela;
	struct section *sec;
	...
	list_for_each_entry(rela, &sec->rela->relas, list)
		livepatch_include_symbol(rela->sym, recurselevel+1);
out:
	inc_printf("end include_symbol(%s)\n", sym->name);
	return;
}

static void livepatch_include_standard_elements(struct livepatch_elf *kelf)
{
	struct section *sec;
	...
	list_entry(kelf->symbols.n.next, struct symbol, list)->include = 1;
}

static int livepatch_include_new_globals(struct livepatch_elf *kelf)
{
	struct symbol *sym;
	int nr = 0;
	...
	list_for_each_entry(sym, &kelf->symbols, list) {
		if (sym->bind == STB_GLOBAL && sym->sec &&
		    sym->status == NEW) {
			livepatch_include_symbol(sym, 0);
			nr++;
		}
	}
...
static int livepatch_include_changed_functions(struct livepatch_elf *kelf)
{
	struct symbol *sym;
	int changed_nr = 0;
...
static void livepatch_print_changes(struct livepatch_elf *kelf)
{
	struct symbol *sym;
...
static void livepatch_migrate_symbols(struct list_head *src,
                                    struct list_head *dst,
                                    int (*select)(struct symbol *))
{
	struct symbol *sym;

	list_for_each_entry(sym, src, list) {
...
static void livepatch_migrate_included_elements(struct livepatch_elf *kelf, struct livepatch_elf **kelfout)
{
	struct section *sec;
	struct symbol *sym;
	struct livepatch_elf *out;
...
static void livepatch_reorder_symbols(struct livepatch_elf *kelf)
{
	LIST_HEAD(symbols);

	/* migrate NULL sym */
	livepatch_migrate_symbols(&kelf->symbols, &symbols, is_null_sym);
	/* migrate LOCAL FILE sym */
	livepatch_migrate_symbols(&kelf->symbols, &symbols, is_file_sym);
	/* migrate LOCAL FUNC syms */
	livepatch_migrate_symbols(&kelf->symbols, &symbols, is_local_func_sym);
	/* migrate all other LOCAL syms */
	livepatch_migrate_symbols(&kelf->symbols, &symbols, is_local_sym);
	/* migrate all other (GLOBAL) syms */
	livepatch_migrate_symbols(&kelf->symbols, &symbols, NULL);

	list_splice_init(&symbols, &kelf->symbols);
}

static void livepatch_reindex_elements(struct livepatch_elf *kelf)
{
	struct section *sec;
	struct symbol *sym;
	int index;
...
static void livepatch_include_debug_sections(struct livepatch_elf *kelf)
{
	struct section *sec;
	struct rela *rela;
...
static void livepatch_process_special_sections(struct livepatch_elf *kelf)
{
	struct section *sec;
	/*
	 * The following special sections aren't supported, so make sure we
	 * don't ever try to include them.  Otherwise the sheep will see the
	 * jump table during patch loading and get confused.  Generally it
	 * should be safe to exclude them, it just means that you can't modify
	 * jump labels in a patched function.
	 */
	list_for_each_entry(sec, &kelf->sections, list) {
		if (strcmp(sec->name, ".eh_frame"))
			continue;
...
static void livepatch_create_shstrtab(struct livepatch_elf *kelf)
{
...
static void livepatch_create_strtab(struct livepatch_elf *kelf)
{
...
static void livepatch_create_symtab(struct livepatch_elf *kelf)
{
	struct section *symtab;
	struct symbol *sym;
	char *buf;
...
static struct section *create_section_pair(struct livepatch_elf *kelf,
                                           char *name, int entsize, int nr)
{
	...
	/* set data, buffers generated by livepatch_rebuild_rela_section_data() */
...
static void livepatch_create_patches_sections(struct livepatch_elf *kelf,
                                    struct lookup_table *table, char *hint)
{
	...
	sec = create_section_pair(kelf, (char *)".livepatch.funcs", sizeof(*funcs), nr);
	relasec = sec->rela;
...
static void livepatch_create_dynamic_rela_sections(struct livepatch_elf *kelf,
                                         struct lookup_table *table, char *hint)
{
	int nr, index;
	struct section *sec, *dynsec, *relasec;
	struct rela *rela, *dynrela;
	struct symbol *strsym;
	struct lookup_result result;
	struct livepatch_patch_dynrela *dynrelas;
	...
	list_for_each_entry(sec, &kelf->sections, list) {
		if (!is_rela_section(sec))
			continue;
		if (!strcmp(sec->name, ".rela.livepatch.funcs"))
			continue;
		list_for_each_entry(rela, &sec->relas, list)
	...
	/* create text/rela section pair */
	dynsec = create_section_pair(kelf, (char *)".livepatch.dynrelas", sizeof(*dynrelas), nr);
	relasec = dynsec->rela;
	...
	list_for_each_entry(sec, &kelf->sections, list) {
		if (!is_rela_section(sec))
			continue;
		if (!strcmp(sec->name, ".rela.livepatch.funcs") ||
		    !strcmp(sec->name, ".rela.livepatch.dynrelas"))
			continue;
		list_for_each_entry(rela, &sec->relas, list) {
			if (rela->sym->sec || !rela->sym->name)
				continue;
			...
			if (rela->sym->bind == STB_LOCAL) {
				 ...
				 * symbol is from a shared library
				 */
			...
			dynrela->offset = index * sizeof(*dynrelas) + offsetof(struct livepatch_patch_dynrela, name);
	...
	dynsec->data->d_size = index * sizeof(struct livepatch_patch_dynrela);
 ...
 */
static void livepatch_strip_unneeded_syms(struct livepatch_elf *kelf,
...
static void livepatch_create_strings_elements(struct livepatch_elf *kelf)
{
	struct section *sec;
	struct symbol *sym;

	/* create .livepatch.strings */

	/* allocate section resources */
	ALLOC_LINK(sec, &kelf->sections);
	sec->name = (char *)".livepatch.strings";
	...
	/* create .livepatch.strings section symbol (reuse sym variable) */
	...
	sym->bind = STB_LOCAL;
	sym->name = (char *)".livepatch.strings";
}

static void livepatch_build_strings_section_data(struct livepatch_elf *kelf)
{
	struct string *string;
	struct section *sec;
	int size;
	...
	if (!sec)
		ERROR("can't find .livepatch.strings");
...
static void livepatch_rebuild_rela_section_data(struct section *sec)
{
	struct rela *rela;
...
static void livepatch_write_output_elf(struct livepatch_elf *kelf, Elf *elf, char *outfile)
{
	int fd;
	struct section *sec;
	Elf *elfout;
...
static char args_doc[] = "original.o patched.o sheep output.o";
 ...
 */
static void livepatch_elf_teardown(struct livepatch_elf *kelf)
{
	struct section *sec;
	struct symbol *sym;
	struct rela *rela;
...
static void livepatch_elf_free(struct livepatch_elf *kelf)
{
	elf_end(kelf->elf);
	close(kelf->fd);
	memset(kelf, 0, sizeof(*kelf));
...
{
	struct livepatch_elf *kelf_base, *kelf_patched, *kelf_out;
	...
	kelf_base = livepatch_elf_open(arguments.args[0]);
	kelf_patched = livepatch_elf_open(arguments.args[1]);

	livepatch_compare_elf_headers(kelf_base->elf, kelf_patched->elf);
	livepatch_check_program_headers(kelf_base->elf);
	livepatch_check_program_headers(kelf_patched->elf);

	livepatch_mark_grouped_sections(kelf_patched);
	livepatch_replace_sections_syms(kelf_base);
	livepatch_replace_sections_syms(kelf_patched);
	livepatch_rename_mangled_functions(kelf_base, kelf_patched);

	livepatch_correlate_elfs(kelf_base, kelf_patched);
	livepatch_correlate_static_local_variables(kelf_base, kelf_patched);
	 ...
	 */
	livepatch_compare_correlated_elements(kelf_patched);
	livepatch_check_fentry_calls(kelf_patched);
	livepatch_elf_teardown(kelf_base);
	livepatch_elf_free(kelf_base);

	livepatch_include_standard_elements(kelf_patched);
	num_changed = livepatch_include_changed_functions(kelf_patched);
	livepatch_include_debug_sections(kelf_patched);
	new_globals_exist = livepatch_include_new_globals(kelf_patched);

	livepatch_print_changes(kelf_patched);
	livepatch_dump_kelf(kelf_patched);

	livepatch_process_special_sections(kelf_patched);
	livepatch_verify_patchability(kelf_patched);
	...
	livepatch_migrate_included_elements(kelf_patched, &kelf_out);
	 ...
	 * livepatch_patched.
	 */
	livepatch_elf_teardown(kelf_patched);
	...
	livepatch_create_strings_elements(kelf_out);
	livepatch_create_patches_sections(kelf_out, lookup, hint);
	livepatch_create_dynamic_rela_sections(kelf_out, lookup, hint);
	livepatch_build_strings_section_data(kelf_out);
	 ...
	 */
	livepatch_reorder_symbols(kelf_out);
	livepatch_strip_unneeded_syms(kelf_out, lookup);
	livepatch_reindex_elements(kelf_out);
	...
	list_for_each_entry(sec, &kelf_out->sections, list) {
		...
		livepatch_rebuild_rela_section_data(sec);
	}

	livepatch_create_shstrtab(kelf_out);
	livepatch_create_strtab(kelf_out);
	livepatch_create_symtab(kelf_out);
	livepatch_dump_kelf(kelf_out);
	livepatch_write_output_elf(kelf_out, kelf_patched->elf, arguments.args[3]);

	livepatch_elf_free(kelf_patched);
	livepatch_elf_teardown(kelf_out);
	livepatch_elf_free(kelf_out);
